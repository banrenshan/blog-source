...
如果 运行的时候不指定-XX:BiasedLockingStartupDelay，表示偏向锁延时开启,打印的结果是：
...
public static void main(String[] args) throws IOException {
   Dog d = new Dog();
   ClassLayout classLayout = ClassLayout.parseInstance(d);
   log.debug("调用 hashCode:{}",d.hashcode());
   new Thread(() -> {
       log.debug("synchronized 前");
       System.out.println(classLayout.toPrintableSimple(true));
       synchronized (d) {
...
如果对象，虽然被多个线程访问，但没有竞争，这时偏向了线程 T1 的对象仍有机会重新偏向 T2，重偏向会重置对象 的 Thread ID。 当撤销偏向锁阈值超过 20 次后，jvm 会这样觉得，我是不是偏向错了呢，于是会在给这些对象加锁时重新偏向至 加锁线程。
...
加锁和不加锁，执行的时间差不多，这是因为jvm识别出来你的加锁代码块并没有多线程访问，做了锁消除。

禁止锁消除：-XX:-EliminateLocks：
